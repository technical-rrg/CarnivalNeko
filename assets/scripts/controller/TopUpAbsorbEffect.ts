 ...
 *   2. +1 Spin: effect bay tu o +1 toi UI spins remaining
 *   3. Yellow hut tien: effect bay tu tung dong Do toi dong Vang;
 *      trong luc bay, dong Vang nhun nhun; khi tat ca toi noi count-up so tien
 *   4. Green hut tien: tuong tu nhung nguon = tat ca dong (Do + Vang + Green khac);
 *      neu nhieu Green dien lan luot trai-phai (Green A xong moi den Green B)
 *   5. Emit TOPUP_ABSORB_DONE cho GameManager tiep tuc
 *
 * LUAT:
 *   - Vang CHI hut Do.
 *   - Xanh hut TAT CA (Do, Vang, Xanh khac).
 ...
 *   4. spinsRemainingNode -> Node UI "Spins Remaining"
 */
...
export class TopUpAbsorbEffect extends Component {
    ...
    @property({
        type: Node,
        tooltip: 'Node mẫu effect phát trên đồng đỏ khi nó bắt đầu bay. Cần có ParticleSystem.',
    })
    redHitTemplate: Node | null = null;

    @property({
        type: Node,
        tooltip: 'Node mẫu effect phát trên đồng vàng: khi đỏ bay vào vàng, hoặc khi vàng bay vào xanh. Cần có ParticleSystem.',
    })
    ...
    @property({
        type: Node,
        tooltip: 'Node UI "Spins Remaining" dich bay cua effect +1 Spin.',
    })
    spinsRemainingNode: Node | null = null;
    ...
    @property({ tooltip: 'Thoi gian icon +1 Spin bay tu grid toi spinsRemainingNode (giay)' })
    plusOneFlyDuration: number = 0.65;
    ...
    private async _onAbsorbStart(payload: TopUpAbsorbPayload): Promise<void> {
        ...
        const { newCells, plusOneSpinCount, allStickyCells, newSpinCount, serverDeltaWin, serverTotalWin } = payload;
        ...
        Log.d(`[TopUpAbsorb] START newCells=${newCells.length} +1spin=${plusOneSpinCount} newSpinCount=${this._newSpinCount}`);
        ...
        try {
            ...
            if (plusOneSpinCount > 0) {
                Log.d(`[TopUpAbsorb] Step1: +1 Spin...`);
            ...
            //    Chi Vang moi duoc lam dich hut; Vang cu da hut roi thi khong reset ve 0/hut lai.
            const newYellowKeys = this._sortedNewKeys(newCells, SymbolId.STICKY_YELLOW);
            for (let yi = 0; yi < newYellowKeys.length; yi++) {
                const yCell = allStickyCells.get(newYellowKeys[yi]);
                if (!yCell) continue;
                const dstNode = this._getSlotNode(yCell.reel, yCell.row);
                if (!dstNode || !dstNode.active) continue;

                // CHI lay dong Do lam nguon
                const sources = this._sortCellsForAbsorbSources(
                    this._getCellsBySymbol(allStickyCells, SymbolId.STICKY_RED)
                        .filter(cell => (cell.credit ?? 0) > 0)
                );
                if (sources.length === 0) continue;

                const creditTarget = sources.reduce((sum, s) => sum + (s.credit ?? 0), 0);
                Log.e(`[TOPUP-CREDIT][ABSORB] yellow dst=${yCell.reel}-${yCell.row} target=${creditTarget} sources=${sources.map(s => `${s.reel}-${s.row}:${SymbolId[s.symbolId] ?? s.symbolId}=${s.credit ?? 0}`).join('|')}`);
                await this._absorbIntoCoin(sources, dstNode, yCell.symbolId, creditTarget, yCell);
                didAbsorbMoneyThisSpin = true;
                if (yi < newYellowKeys.length - 1) await this._delay(this.coinSequenceDelay * this._spd());
            }

            // 3. Green absorb - tuan tu tung dong Xanh moi (trai-phai, tren-duoi)
    ...
    private _flyEffectsWithIncrementalCredit(
        ...
        return new Promise(resolve => {
            ...
            for (let i = 0; i < n; i++) {
                ...
                this.scheduleOnce(() => {
                    if (srcSymbolId === SymbolId.STICKY_RED) {
                        this._spawnHitEffect(this._redHitPool, srcNode.worldPosition);
                    } else if (srcSymbolId === SymbolId.STICKY_YELLOW) {
    ...
    // ==========================================================================
    //  STEP 2 - +1 Spin: chi cap nhat so freespin tren UI, khong bay icon +1
    // ==========================================================================

    private async _stepPlusOneSpin(newCells: StickyCell[], count: number): Promise<void> {
        const plusOneCells = newCells.filter(c => c.symbolId === SymbolId.PLUS_ONE_SPIN);
        const countNode = this._getSpinsRemainingNode();
        if (this.stickyOverlay) {
            this.stickyOverlay.clearTempPlusOne('before-plus-step');
        }
        if (count <= 0) {
            if (plusOneCells.length > 0) {
                Log.e(`[TOPUP-PLUS] SKIP visual +1 because count=0 cells=${plusOneCells.map(c => `${c.reel}-${c.row}`).join('|')}`);
            }
            return;
        }
        Log.e(
            `[TOPUP-PLUS] absorbPlusStep count=${count} cells=${plusOneCells.length}` +
            ` newSpinCount=${this._newSpinCount}` +
            ` keys=${plusOneCells.map(c => `${c.reel}-${c.row}`).join('|') || 'none'}` +
            ` bounceTarget=${countNode?.name ?? 'none'} active=${countNode?.activeInHierarchy ? 1 : 0}`
        );
        let didEmitCount = false;
        const emitCountOnce = (reason: string) => {
            if (didEmitCount || this._newSpinCount < 0) return;
            didEmitCount = true;
            Log.e(`[TOPUP-PLUS] emit TOPUP_COUNT_UPDATED(${this._newSpinCount}) ${reason}`);
            EventBus.instance.emit(GameEvents.TOPUP_COUNT_UPDATED, this._newSpinCount);
        };

        for (const cell of plusOneCells) {
            // Dung showTempCoin de kich hoat coin slot (co the dang inactive)
            let slotNode: Node | null = null;
            if (this.stickyOverlay) {
                slotNode = this.stickyOverlay.showTempCoin(cell.reel, cell.row, cell.symbolId, true);
            }
            if (!slotNode) {
                slotNode = this._getSlotNode(cell.reel, cell.row);
            }
            Log.d(`[TopUpAbsorb] +1Spin cell reel=${cell.reel} row=${cell.row} slotNode=${slotNode?.name} active=${slotNode?.active}`);
            Log.e(`[TOPUP-PLUS] play visual +1 key=${cell.reel}-${cell.row} slot=${slotNode?.name ?? 'none'} active=${slotNode?.active ? 1 : 0}`);
            ...
            if (slotNode && isValid(slotNode) && slotNode.active) {
                ...
                // Emit số spin cập nhật ngay trước khi nhún
                emitCountOnce('with visual');

                // ★ Nhún song song: cả +1 symbol trên overlay và spinsRemainingNode cùng lúc
                const bouncePromises: Promise<void>[] = [];
                bouncePromises.push(this._bounceCoin(slotNode));

                if (countNode && isValid(countNode)) {
                    bouncePromises.push(this._bounceCoin(countNode));
                }

                await Promise.all(bouncePromises);

                // Symbol +1 nằm yên trên grid cho đến lượt spin tiếp theo
                // (StickyOverlay sẽ ẩn khi REELS_START_SPIN xóa nó khỏi stickyCells)
            } else {
                // Fallback: khong co slot -> chi bounce spinsRemainingNode
                emitCountOnce('visual slot missing');
                if (countNode && isValid(countNode)) {
                    await this._bounceCoin(countNode);
                } else {
                    Log.e(`[TOPUP-PLUS] missing spinsRemainingNode; count updated but no UI bounce`);
                }
            }
        }

        // Neu server tang count nhung khong co cell +1 cu the, giu internal count va khong nhay UI.
        if (plusOneCells.length === 0 && count > 0) {
            Log.e(`[TOPUP-PLUS] SUPPRESS_NO_VISUAL_COUNT count=${count} newSpinCount=${this._newSpinCount} reason=noPlusOneCell`);
            return;
        }

        await this._delay(0.6 * this._spd());
    }

    private _getSpinsRemainingNode(): Node | null {
        if (this.spinsRemainingNode && isValid(this.spinsRemainingNode)) {
            return this.spinsRemainingNode;
        }
        const ui = this.node.scene?.getComponentInChildren(TopUpUI);
        const node = ui?.spinsRemainingSpriteNumber?.node ?? null;
        if (node && isValid(node)) {
            this.spinsRemainingNode = node;
            Log.e(`[TOPUP-PLUS] autoBind spinsRemainingNode=${node.name}`);
            return node;
        }
        return null;
    }
    ...
    /**
     * Clone srcNode, bay clone tu vi tri srcNode den vi tri dstNode tren overlay layer
     * theo duong cong bezier. Node goc KHONG bi dich chuyen.
     * Clone scale-down ve 0 khi toi dich roi moi destroy.
     */
    private _flyCloneTo(srcNode: Node, dstNode: Node): Promise<void> {
        return new Promise(resolve => {
            if (!isValid(srcNode) || !isValid(dstNode) || !isValid(this.node)) {
                resolve();
                return;
            ...
            clone.name = `${srcNode.name}_flyClone`;
            clone.setParent(this.node);

            const startWorld = srcNode.getWorldPosition();
            const startLocal = layerUT.convertToNodeSpaceAR(startWorld);
            clone.setPosition(startLocal.x, startLocal.y, 0);
            clone.setScale(1, 1, 1);
            clone.active = true;
            clone.setSiblingIndex(this.node.children.length - 1);

            const endLocal = layerUT.convertToNodeSpaceAR(dstNode.getWorldPosition());

            // ── Duong cong bezier bac 2 ──────────────────────────────────────
            // Control point: offset vuong goc tai diem giua duong thang src->dst
            const dx = endLocal.x - startLocal.x;
            const dy = endLocal.y - startLocal.y;
            const len = Math.sqrt(dx * dx + dy * dy) || 1;
            const arcOffset = Math.min(len * 0.4, 160);
            const midX = (startLocal.x + endLocal.x) / 2;
            const midY = (startLocal.y + endLocal.y) / 2;
            // Perpendicular vector (-dy, dx) normalized
            const ctrlX = midX + (-dy / len) * arcOffset;
            const ctrlY = midY + ( dx / len) * arcOffset;

            const bezierPt = (t: number): Vec3 => {
                const mt = 1 - t;
                return new Vec3(
                    mt * mt * startLocal.x + 2 * mt * t * ctrlX + t * t * endLocal.x,
                    mt * mt * startLocal.y + 2 * mt * t * ctrlY + t * t * endLocal.y,
                    0
                );
            };

            const p1 = bezierPt(0.33);
            const p2 = bezierPt(0.67);
            const p3 = bezierPt(1.0);

            const flyDur = this.plusOneFlyDuration * this._spd();
            const seg    = flyDur / 3;
            ...
            const doResolve = () => {
                if (!resolved) {
                    resolved = true;
                    if (isValid(clone)) clone.destroy();
                    resolve();
                }
            };
            ...
            tween(clone)
                // Bay theo 3 doan cua bezier
                .to(seg, { position: p1 }, { easing: 'sineOut' })
                .to(seg, { position: p2 }, { easing: 'linear' })
                .to(seg, { position: p3 }, { easing: 'sineIn' })
                // Toi sat dich: pop nho len roi thu nho ve 0 moi mat
                .to(0.07, { scale: new Vec3(1.2, 1.2, 1) }, { easing: 'sineOut' })
                .to(0.13, { scale: new Vec3(0,   0,   1) }, { easing: 'sineIn' })
                .call(() => {
                    SoundManager.instance?.playBonusTrail();
                    doResolve();
                })
            ...
            // Failsafe
            this.scheduleOnce(doResolve, flyDur + 0.5);
        });
    }
